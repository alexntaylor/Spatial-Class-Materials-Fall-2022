...
# Voronoi Diagrams---These can be useful. 
# (1) You want "random" variation in a border you already have. 
# (2) You want a "natural" unit to use in your analysis.
...
# Open the GREG map of ethnic groups (./greg_afg/GREG_Afghanistan_Box.shp) 
# and the Digital Chart of the World (/vmap0_boundaries/world-bounds-a.shp). 
# Make sure to set the CRS to EPSG 4326. Subset the latter to Afghanistan.
...
# Intersect the GREG polygons with the DCW map of Afghanistan. 
# Compute the area of each polygon and delete those smaller than 1 km2.
# Keep only the variables GREGID, GREGNAME and a new variable for the 
# country name, called COUNTRY.
...
# Plot the result to achieve replication goal 1. 
# To get a unique color for each group, you might want to 
# specify plot(..., pal= sf.colors(x)) where x is the number of groups.
...
# Load the F142000 and F152000 light rasters from the folder 
# ntl_2000. Crop both rasters to the spatial extent of the 
# GREG-DCW intersection you computed above. Create a new 
# raster which contains the average of the two rasters. 
# Divide this new raster by the area of each cell and check
# what the new maximum is using cellStats().
...
# Use raster::extract() to compute the average light density in each 
# ethnic homeland. Call the new variable light and merge it back to 
# the GREG-DCW feature.

light.df <- raster::extract(r.light, greg.dcw.sp, fun=mean, na.rm=T, df=T)
light.df <- light.df %>% rename(light = 2)
greg.dcw.sf <- greg.dcw.sf %>% mutate(ID = 1:nrow(.)) %>%
  left_join(light.df, by = "ID")

# Open the Gridded Population of the World (GPW) v3 population 
# density raster for the year 2000 (./gpw3_pdens_2000/glds00g.bil) 
# and set the CRS to match the GREG-DCW features. Then extract the 
...
# by the population density. Plot this variable on a map using 
# plot(..., breaks="kmeans"). Use the gini()-function to compute 
# the unweighted and population-weighted Gini coefficient of light per capita.
# Which Gini coefficient is more meaningful in this context? 
# How do your results compare to Alesina et al. (2016)?
...
gini(greg.dcw.sf[["light_pc"]])


# Create a grid of 2.5° cells starting from Pmin = (−180,−65) in the
# lower left corner. Intersect this grid with the DCW for Afghanistan 
# and plot the result to verify it them.
...
# Extract the light and population densities in each grid cell and 
# add them back to the original feature. Calculate light per capita, 
# plot the result and compute the unweighted and weighted Gini coefficients. 
# Compare your results to Alesina et al. (2016).
...
# Project the GREG-DCW Afghanistan feature into the Azimuthal Equidistant 
# Projection and then union the polygons to obtain the full polygon of 
# Afghanistan again. Create a Voronoi diagram on the basis of the 
# projected GREG-DCW feature, that is, compute the centroids, 
# combine the features, call the st_voronoi-function, cast the 
# result into single polygons, use st_sf() to merge back the original 
# data and IDs, intersect this with the full polygon of Afghanistan, 
# and finally transform the result back to EPSG 4326. How many Thiessen 
# homelands do you have? Plot the result to verify.
...
# Extract the light and population densities in each Thiessen homeland 
# and add them back to the original feature. Calculate light per capita,
# plot the result and compute the unweighted and weighted Gini coefficients.
# Compare your results to Alesina et al. (2016).
